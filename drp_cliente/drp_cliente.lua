...
Citizen.CreateThread(function()
	while true do
		...
		if cauldron or still then 
			TriggerEvent('rvorp:Tip', "[G] destilar alcohol", 100)
...
Citizen.CreateThread(function()
    WarMenu.CreateMenu('still', "Destilar")
    WarMenu.SetSubTitle('still', 'Destilar')
    WarMenu.CreateSubMenu('prepare', 'still', 'Destilar Alcohol')
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('still') then
            if WarMenu.MenuButton('preparar', 'prepare') then
            end
            WarMenu.Display()
        elseif WarMenu.IsMenuOpened('prepare') then
            if WarMenu.Button('Destilando Alcohol') then
...
AddEventHandler('shiner:moonshine', function()
    ...
    local playerPed = PlayerPedId()
    exports['progressBars']:startUI(27000, "Destilando alcohol...")
